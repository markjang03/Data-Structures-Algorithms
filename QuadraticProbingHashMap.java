...
public class QuadraticProbingHashMap<K, V> {
    ...
    public V put(K key, V value) {
        if (key == null || value == null) {
            throw new IllegalArgumentException("The key or value is null; we can't put null keys or null values.");
        }
        ...
        if (removedIndex != -1) {
            table[removedIndex] = new QuadraticProbingMapEntry<>(key, value);
        } else {
    ...
    public V remove(K key) {
        if (key == null) {
            throw new IllegalArgumentException("the key is null; we can't remove null keys.");
        }
        ...
        throw new NoSuchElementException("key doesnt exist in the table.");
    }
    /**
     * Finds the index of input key in a hash table.
     *
     * This method searches for the input key in the hash table. If the key is found,
     * it returns the index of the key in the table. If the key is not found but there
     * is a previously removed element in the table, it returns the index of the removed element.
     * If the key is not found and there are no removed elements, it returns -1.
     *
     * @param key key to search index
     * @return the index of the key input
     */
    private int findKeyIndex(K key) {
        ...
        while (table[hash] != null && probeCount < table.length) {
            ...
            hash = (hash + (probeCount + 1) * (probeCount + 1)) % table.length;
            probeCount++;
        }
        if (removedIndex != -1) {
            return removedIndex;
    ...
    public V get(K key) {
        if (key == null) {
            throw new IllegalArgumentException("The key is null; we can't get values with null keys.");
        }
        int index = findKeyIndex(key);
        if (index != -1) {
    ...
    public void resizeBackingTable(int length) {
        if (length < 0 || length < size) {
            throw new IllegalArgumentException("Length is smaller than 0 or size.");
        }
        QuadraticProbingMapEntry<K, V>[] entries = new QuadraticProbingMapEntry[length];
